...
#define motorcontroliotMESSAGE                            "{\"" "Velocity" "\":%0.2f,\n\"" "Current" "\":%0.2f,\n\"" "Voltage" "\":%0.2f}"

extern void getData(float* position, float* current, float* velocity);
...
{   
    ...
    getData(&velocity, &position, &current);


    int result = snprintf( ( char * ) pucTelemetryData, ulTelemetryDataSize,
                           motorcontroliotMESSAGE,  position, velocity, current);
...
{
    LogInfo(("Inside sample_azure_iot_pnp_simulated_data.c: ulCreateReportedPropertiesUpdate"));
