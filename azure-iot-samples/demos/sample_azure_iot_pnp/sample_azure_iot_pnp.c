...
/**
 * @brief Delay (in ticks) between 
 */
#define sampleazureiotDELAY_BETWEEN_DATA_ACQUISITION_TICKS          ( pdMS_TO_TICKS( 1000U ) )
    ...
    for( ; ; )
    {
        if( xAzureSample_IsConnectedToInternet() )
        {
            ...
            for( ; xAzureSample_IsConnectedToInternet(); )
            {
                ...
                LogInfo(("inside prvAzureDemoTask in sample_azure_iot_pnp.c: right before ulCreateReportedPropertiesUpdate"));
...
/**
 * @brief Getting data from the motor, to store in memory until the Azure networking task is called
 * 
 */
static void prvDataAcqTask( void * pvParameters ) {
    for(;;){
        LogInfo(("testing the task"));
        vTaskDelay(sampleazureiotDELAY_BETWEEN_DATA_ACQUISITION_TICKS);
    }
}
...
{
    LogInfo(("vStartDemoTask in: sample_azure_iot_pnp.c"));
...
void vStartDataAcqTask(void)
{
    LogInfo(("vStartDataAcqTask in: sample_azure_iot_pnp.c"));
    xTaskCreate( prvDataAcqTask,         /* Function that implements the task. */
                "DataAcqTask",          /* Text name for the task - only used for debugging. */
